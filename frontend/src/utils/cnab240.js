...
export function buildCNAB240(form) {
  ...
  const hArq = [
    ...
    spaces(20),                                     // 172-191 Reservado banco (branco)
    spaces(20),                                     // 192-211
    spaces(29),                                     // 212-240
  ].join("");
  lines.push(hArq);
  ...
  for (const pgt of pagamentos) {
    ...
    // Segmento A (TED / Crédito em Conta)
    const segA = [
      banco,                                            // 1-3
      lotePad,                                          // 4-7
      "3",                                              // 8
      ...
      padLeft(pgt.quantidadeMoeda || "0", 15),          // 105-119
      fmtValue(pgt.valor, 13, 2),                       // 120-134
      padRight(pgt.nossoNumero || "", 20),              // 135-154
      fmtDate(pgt.dataRealPagamento || pgt.dataPagamento), // 155-162
      fmtValue(pgt.valorRealPagamento || pgt.valor || "0", 13, 2), // 163-177
    ...
    // Segmento B (TED - endereco e valores do favorecido)
